...
class Tree:
    ...
    def get(
        ...
        if mode == "input":
            ...
            min_input_freq = sorted(freqs, key=lambda x: x[1], reverse=True)[min_input_size - 1][1] if size > max_size else 0.0
        elif mode == "output":
            output_weight = 1.0
            size = len([x for x in freqs if x[2] > 0])
            min_output_freq = (
                sorted(freqs, key=lambda x: x[2], reverse=True)[min_output_size - 1][2] if size > max_size else 0.0
            )
        else:
    ...
    def _ravel(
        ...
        sorts = sorted(
            [
                (
                    k,
                    v,
                    (1.0 - output_weight) * v.freqs.get(idx, 0.0) + output_weight * v.freqs.get(-1, 0.0),
                )
        ...
        for tid, node, fm in sorts:
            ...
            fo = node.freqs.get(-1, 0.0)
            if mode == "mix" and fi < min_input_freq and fo < min_output_freq and fm < min_mix_freq:
                continue
    ...
    def one_get(
        ...
        for i, t in enumerate(token_ids):
            ...
            if tree is not None:
                ...
                if t in self.stop_words and len(ids) == 0:
                    continue
                decoding_ids, sizes = tree.get_one_branch(ids, max_length=branch_length - 1, mode=mode, idx=idx)
                s = len(decoding_ids)
