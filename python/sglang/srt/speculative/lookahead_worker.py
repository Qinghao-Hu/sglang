...
class LOOKAHEADWorker:
    ...
    def __init__(
        ...
        if tp_rank == 0:
            self.lookahead_cache = LookaheadCache(debug=False, eos_ids=None, gpu_id=tp_rank)
    ...
    def prepare_for_verify(self, batch: ScheduleBatch):
        ...
        for i, (req_drafts, mask_) in enumerate(look_ahead_res):
            ...
            req_mask = torch.ones((len(req_drafts), seq_len - 1)).cuda()  # TODO: check the new generated token
        ...
        retrive_cum_len = torch.zeros((leaf_nums.numel() + 1,), dtype=torch.int32, device="cuda")
    ...
    def finish_request(self, reqs: Union[Req, List[Req]]):
        ...
        for req in reqs:
            if self.lookahead_cache is not None:
                put_ids = req.origin_input_ids[-self.num_branch_token :] + req.output_ids
